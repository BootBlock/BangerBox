<#
  ...
  unified flow:
    ...
    1. Is a BangerBox server already answering on the dev port? If so it is ready —
  ...
  Cross-origin isolation: BangerBox needs a cross-origin-isolated context for
  SharedArrayBuffer and the SQLite OPFS VFS (spec §2.3.1). The dev and preview servers
  set the COOP/COEP headers directly (vite.config.ts) — this launcher confirms a candidate
  server really is BangerBox by checking for that COOP header before reusing or opening it.
  ...
  Pass -BindHost localhost (or set $env:BANGERBOX_DEV_HOST=localhost) to keep the localhost
  ...
  -NoOpen (equivalently -Browser none / $env:BROWSER='none').
#>
[CmdletBinding()]
param(
  ...
  # reliability; see the header comment). $env:BANGERBOX_DEV_HOST overrides the default.
  [string]$BindHost = $(if ($env:BANGERBOX_DEV_HOST) { $env:BANGERBOX_DEV_HOST } else { '127.0.0.1' }),
...
# BangerBox serves from the site root (manifest scope/start_url '/', spec §2.4).
$BasePath = '/'
...
# Is a *BangerBox* server already answering on this port and serving the page? Confirmed
# via the distinctive cross-origin-isolation header the app serves (spec §2.3.1) plus
# the app shell, so we never reuse — or "open against" — an unrelated server that
# merely holds the port.
function Test-BangerBoxServer([int]$Port, [string]$UrlHost) {
  try {
    ...
    return ($resp.StatusCode -eq 200 -and $coop -eq 'same-origin' -and $resp.Content -match 'BangerBox')
  }
  catch {
    return $false
  }
...
Write-Host '  BangerBox - offline-first browser DAW & sampler' -ForegroundColor Cyan
Write-Host '===================================================' -ForegroundColor Cyan
Write-Host ''
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
  ...
  Write-Host 'Install Node.js 24 or newer from https://nodejs.org then run this again.' -ForegroundColor Red
  exit 1
}
...
if ($Mode -eq 'preview') {
  ...
  Write-Host "Starting BangerBox (preview) at $url" -ForegroundColor Green
...
if (Test-PortListening -Port $defaultPort) {
  if (Test-BangerBoxServer -Port $defaultPort -UrlHost $UrlHost) {
    $reuse = $true
    $port = $defaultPort
  }
  else {
...
# (1) A BangerBox server is already up and ready — just open the browser there.
if ($reuse) {
  Write-Host "BangerBox is already running at $url" -ForegroundColor Green
  if ($AutoOpen) {
...
Write-Host "Starting BangerBox at $url" -ForegroundColor Green
Write-Host $ReadyHint -ForegroundColor DarkGray
Write-Host 'Press Ctrl+C in this window to stop the server (avoid the [X] button).' -ForegroundColor DarkGray
Write-Host ''
