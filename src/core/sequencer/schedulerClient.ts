...
export class SchedulerClient {
  ...
  sendEventsDiff(trackId: string, sequenceId: string, upserts: readonly MidiEvent[], deletes: readonly string[]): void {
    this.#send({ kind: 'eventsDiff', trackId, sequenceId, upserts, deletes });
  }
  sendAutomationDiff(scope: AutomationPoint['scope'], ownerId: string, targetPath: string, points: readonly AutomationPoint[]): void {
  ...
  setSequenceMeta(sequences: Readonly<Record<string, SchedulerSequenceMeta>>, projectBpm: number, activeSequenceId: string | null, playbackMode: 'sequence' | 'song'): void {
