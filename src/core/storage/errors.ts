...
export interface SerialisedDbError {
...
export class DbError extends Error {
  ...
  toSerialised(): SerialisedDbError {
    return {
      name: 'DbError',
      code: this.code,
  ...
  static fromSerialised(error: SerialisedDbError): DbError {
...
export function isSerialisedDbError(value: unknown): value is SerialisedDbError {
  return (
    typeof value === 'object' &&
    value !== null &&
...
function extractResultCode(error: unknown): number | undefined {
  ...
  // node:sqlite (test driver) surfaces `errcode` — accept either.
  const candidate =
    (error as { resultCode?: unknown }).resultCode ?? (error as { errcode?: unknown }).errcode;
