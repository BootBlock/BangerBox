...
6. **SQLite distribution: official `@sqlite.org/sqlite-wasm`**, worker-hosted, on the OPFS VFS (SharedArrayBuffer-coordinated), exactly as proven in the Gubbins reference implementation. No wa-sqlite, no sql.js, no IndexedDB fallback for primary data.
...
`node_modules/`, or the working usage in the Gubbins reference repo (ask the human
developer for its location; adapt patterns, never copy branding or domain logic).
...
- **2026-07-18 (l)** — _Pre-publication redaction, directed by the human developer._ Absolute local filesystem paths were removed ahead of the repository's first public push: the checkout location of the Gubbins reference repository (§1.3 #6, §2.7, and the §13.7 session-prompt template) and of the BangerBox checkout itself (§13.7). They recorded a directory layout on the developer's machine, and the former also named a separate, unpublished project — the kind of environment detail §1.3 #3 keeps out of a public repo. The §13.7 template now tells the incoming session to work in the repository root it was opened in, which is how every session has actually started. **The reference-implementation rule itself is unchanged and still binding** (§13.6): the Gubbins repo remains the house reference for the DB worker, RPC bridge, `sw.ts`, `vite.config.ts`, and the PWA update prompt, and MUST still be consulted before those are written. Only the checkout location is no longer recorded here; a session that needs it asks the human developer (§13.3.2). No technical constraint, decision, or code changed. Added at the same time: a root `LICENSE` (MIT, matching the `license` field `package.json` has always carried) and a public `README.md` — neither is a spec artefact, and neither supersedes any part of this document.
