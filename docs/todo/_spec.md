...
8. AudioWorklet processor modules are built as real files (via `new URL('./x.worklet.ts', import.meta.url)` worker/worklet handling), never inlined as blob URLs — blob workers break under strict COEP.
